...
```
### 14. Difference between defer and async
- Defer :
    - The defer attribute is similar to async in that it also loads the script in parallel with parsing the HTML. However, the difference is that scripts with the defer attribute are executed in the order they appear in the document, after the HTML parsing is complete.
